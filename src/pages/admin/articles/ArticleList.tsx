...
export default function ArticleList() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get<PaginatedResponse<Article>>('/admin/articles');
      return res.data.data.data;
    }
  });

  const deleteMutation = useMutation({
    mutationFn: async (id: number) => await api.delete(`/admin/articles/${id}`),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6 relative">
      ...
      <div className="flex justify-between items-center bg-white dark:bg-abyss-light p-6 rounded-2xl border border-zinc-200 dark:border-white/10 shadow-sm">
        <div>
          <h2 className="text-2xl font-bold text-zinc-900 dark:text-white">DevLog Artikel</h2>
          <p className="text-sm text-zinc-500">Kelola catatan dan jurnal pengembangan.</p>
        </div>
        <Link to="/admin/articles/create" className="flex items-center gap-2 bg-techblue text-white px-5 py-2.5 rounded-xl font-medium shadow-lg hover:bg-techblue-hover transition-all">
        ...
        {isLoading ? (
          ...
          <table className="w-full text-left text-sm text-zinc-600 dark:text-zinc-300">
            ...
            <tbody className="divide-y divide-zinc-200 dark:divide-white/5">
              {data?.map(article => (
                <tr key={article.id} className="hover:bg-zinc-50 dark:hover:bg-white/[0.02]">
                  <td className="px-6 py-4 font-bold text-zinc-900 dark:text-white line-clamp-1">{article.title}</td>
                  <td className="px-6 py-4">{article.category}</td>
                  <td className="px-6 py-4">
                    <span className={`px-2.5 py-1 rounded-md border font-bold text-xs ${article.status === 'published' ? 'bg-techblue/10 text-techblue border-techblue/20' : 'bg-zinc-100 dark:bg-white/5 border-zinc-200 dark:border-white/10'}`}>
                      {article.status.toUpperCase()}
                    </span>
                  </td>
                  <td className="px-6 py-4 flex justify-end gap-2">
                    <Link to={`/admin/articles/edit/${article.id}`} className="p-2 text-zinc-500 hover:text-techblue hover:bg-techblue/10 rounded-lg transition-colors"><Edit className="w-4 h-4" /></Link>
                    <button onClick={() => setDeleteModal(article.id)} className="p-2 text-red-500 hover:bg-red-500/10 rounded-lg transition-colors"><Trash2 className="w-4 h-4" /></button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {deleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-white dark:bg-abyss-light w-full max-w-sm rounded-2xl p-6 text-center shadow-2xl border border-zinc-200 dark:border-white/10">
            ...
            <h3 className="text-xl font-bold text-zinc-900 dark:text-white mb-2">Hapus Artikel?</h3>
            <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-6">Artikel ini tidak dapat dikembalikan setelah dihapus.</p>
