...
export default function ProjectCreate() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setToastMsg('Proyek berhasil dipublikasikan!');
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-abyss-light p-8 rounded-3xl border border-zinc-200 dark:border-white/10 shadow-lg space-y-8">
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className="col-span-2 md:col-span-1">
            ...
            <input required type="text" onChange={e => setFormData({...formData, title: e.target.value})} className="w-full p-3 rounded-xl border dark:border-white/10 bg-zinc-50 dark:bg-abyss dark:text-white outline-none focus:ring-2 focus:ring-techblue" />
          </div>
          ...
          <div className="col-span-2 md:col-span-1">
            ...
            <div className="relative">
              ...
              <input required type="url" placeholder="https://..." onChange={e => setFormData({...formData, thumbnail_url: e.target.value})} className="w-full pl-10 p-3 rounded-xl border dark:border-white/10 bg-zinc-50 dark:bg-abyss dark:text-white outline-none focus:ring-2 focus:ring-techblue" />
            </div>
          </div>

          <div className="col-span-2">
            <label className="block text-sm mb-2 font-bold dark:text-zinc-300">Tagline Singkat</label>
            <input type="text" placeholder="Sistem e-office responsif..." onChange={e => setFormData({...formData, tagline: e.target.value})} className="w-full p-3 rounded-xl border dark:border-white/10 bg-zinc-50 dark:bg-abyss dark:text-white outline-none focus:ring-2 focus:ring-techblue" />
          </div>

          <div>
            <label className="block text-sm mb-2 font-bold dark:text-zinc-300">Season</label>
            <input required type="number" min="1" onChange={e => setFormData({...formData, season: Number(e.target.value)})} className="w-full p-3 rounded-xl border dark:border-white/10 bg-zinc-50 dark:bg-abyss dark:text-white outline-none focus:ring-2 focus:ring-techblue" />
          </div>
          
          <div>
            <label className="block text-sm mb-2 font-bold dark:text-zinc-300">Status</label>
            <select onChange={e => setFormData({...formData, status: e.target.value})} className="w-full p-3 rounded-xl border dark:border-white/10 bg-zinc-50 dark:bg-abyss dark:text-white outline-none focus:ring-2 focus:ring-techblue">
          ...
          <div>
            ...
            <input type="url" placeholder="https://..." onChange={e => setFormData({...formData, demo_link: e.target.value})} className="w-full p-3 rounded-xl border dark:border-white/10 bg-zinc-50 dark:bg-abyss dark:text-white outline-none focus:ring-2 focus:ring-techblue" />
          </div>
          <div>
            <label className="block text-sm mb-2 font-bold dark:text-zinc-300">Link GitHub (Opsional)</label>
            <input type="url" placeholder="https://github.com/..." onChange={e => setFormData({...formData, github_link: e.target.value})} className="w-full p-3 rounded-xl border dark:border-white/10 bg-zinc-50 dark:bg-abyss dark:text-white outline-none focus:ring-2 focus:ring-techblue" />
          </div>
        ...
        <button type="submit" disabled={loading} className="w-full py-4 bg-techblue hover:bg-techblue-hover text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-colors">
